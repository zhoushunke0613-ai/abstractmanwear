...
function CommercialConcept() {
  ...
  const products = [
    { name: product("boxerBriefs"), note: product("boxerBriefsTag"), image: "/images/product-boxer-briefs.svg", tone: "bg-[#ffc91c]" },
    { name: product("trunks"), note: product("trunksTag"), image: "/images/product-trunks.svg", tone: "bg-[#ff633f]" },
    { name: product("briefs"), note: product("briefsTag"), image: "/images/product-briefs.svg", tone: "bg-[#d8f3e7]" },
    { name: product("performance"), note: product("performanceTag"), image: "/images/product-performance.svg", tone: "bg-[#b9b5ff]" },
  ];
  const serviceCards = [
    { title: services("s1Title"), desc: services("s1Desc"), image: "/images/brand/packing-floor-wide.jpg" },
  ...
  return (
    <div className="concept-commercial min-h-screen overflow-hidden bg-[#f0f0ea] font-sans text-[#121212]">
      ...
      <main>
          ...
          <div className="relative mx-auto grid min-h-[calc(100svh-6rem)] max-w-[96rem] overflow-hidden rounded-[2rem] bg-[#171717] text-white lg:grid-cols-[0.9fr_1.1fr]">
            <div className="relative z-20 flex flex-col justify-between p-7 sm:p-10 lg:p-14 xl:p-16">
              <p className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.2em] text-white/60">
                <span className="h-2.5 w-2.5 rounded-full bg-[#ffc91c]" />{hero("eyebrow")}
              </p>
              <div className="py-14 lg:py-8">
                ...
                <div className="mt-9 flex flex-wrap items-center gap-4">
                  <Link href="/contact" className="inline-flex items-center gap-3 rounded-full bg-[#ffc91c] px-6 py-4 text-xs font-black uppercase tracking-[0.08em] text-black">{hero("startProject")}{arrowIcon}</Link>
            ...
            <div className="relative min-h-[32rem] lg:min-h-full">
              ...
              <div className="absolute right-7 top-8 z-20 rounded-full bg-[#ffc91c] px-5 py-3 text-[10px] font-black uppercase tracking-[0.14em] text-black sm:right-10 sm:top-10">Since 2006</div>
            </div>
          </div>
        </section>

        <section className="px-4 sm:px-6 lg:px-10">
          <dl className="mx-auto grid max-w-[96rem] grid-cols-2 overflow-hidden rounded-[2rem] bg-[#ffc91c] lg:grid-cols-4">
          ...
          <div className="mx-auto max-w-[96rem]">
            ...
            <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {products.map((item, index) => (
                <Link href="/products" key={item.name} className={`${item.tone} group flex min-h-[27rem] flex-col justify-between overflow-hidden rounded-[2rem] p-6 sm:min-h-[32rem] sm:p-8 ${index === 0 ? "lg:translate-y-8" : index === 2 ? "lg:-translate-y-5" : ""}`}>
          ...
          <div className="mx-auto max-w-[96rem]">
            ...
            <div className="mt-16 grid gap-5 md:grid-cols-2">
              {serviceCards.map((item) => (
                <article key={item.title} className="overflow-hidden rounded-[2rem] bg-[#292929]">
        ...
        <section id="commercial-factory" className="bg-[#ff633f] px-4 py-24 sm:px-6 lg:px-10 lg:py-36">
          <div className="mx-auto grid max-w-[96rem] gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            <div><p className="text-xs font-black uppercase tracking-[0.18em]">{capability("eyebrow")}</p><h2 className="mt-5 text-[clamp(3rem,5.5vw,6.2rem)] font-black leading-[0.88] tracking-[-0.065em]">{capability("headline")}</h2><p className="mt-7 max-w-xl text-base font-medium leading-8 text-black/65">{capability("description")}</p><Link href="/capability" className="mt-9 inline-flex items-center gap-3 rounded-full bg-black px-6 py-4 text-xs font-black uppercase tracking-[0.08em] text-white">{capability("tourCta")}{arrowIcon}</Link></div>
        ...
        <section className="bg-[#f0f0ea] px-4 py-24 sm:px-6 lg:px-10 lg:py-36">
          <div className="mx-auto max-w-[96rem]">
            ...
            <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {["s1", "s3", "s4", "s6"].map((key) => (
                <article key={key} className="rounded-[2rem] bg-white p-7 sm:p-8"><span className="mb-14 block h-3 w-3 rounded-full bg-[#ffc91c]" /><h3 className="text-2xl font-black tracking-[-0.03em]">{process(`${key}Title`)}</h3><p className="mt-4 text-sm leading-7 text-black/55">{process(`${key}Desc`)}</p></article>
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="grid min-h-[44rem] lg:grid-cols-2">
          ...
          <div className="flex flex-col justify-center bg-[#b9b5ff] p-8 sm:p-12 lg:p-20"><p className="text-xs font-black uppercase tracking-[0.18em]">{cases("casesEyebrow")}</p><h2 className="mt-5 text-[clamp(3rem,5vw,5.8rem)] font-black leading-[0.9] tracking-[-0.06em]">{cases("casesHeadline")}</h2><p className="mt-7 max-w-xl text-base font-medium leading-8 text-black/60">{cases("casesDesc")}</p><p className="mt-10 max-w-xl border-t-2 border-black pt-7 text-xl font-black">{cases("case3Client")} · {cases("case3P1Title")}</p></div>
        </section>

        <section className="bg-[#ffc91c] px-4 py-24 sm:px-6 lg:px-10 lg:py-32">
...
function CinematicConcept() {
  ...
  return (
    <div className="concept-cinematic min-h-screen overflow-hidden bg-[#ede8df] font-sans text-[#211d19]">
      ...
      <main>
        <section className="relative min-h-[100svh] text-white">
          <Image src="/images/brand/packing-floor-wide.jpg" alt="" fill loading="eager" sizes="100vw" className="object-cover object-center" />
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(13,18,21,.75)_0%,rgba(13,18,21,.16)_58%,rgba(13,18,21,.35)_100%)]" />
        ...
        <section className="bg-[#0d1215] text-white">
        ...
        <section id="cinematic-story" className="py-28 lg:py-44">
          <div className="mx-auto max-w-[104rem] px-6 lg:px-10"><p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#8e583f]">{services("eyebrow")}</p><h2 className="mt-6 max-w-[18ch] text-[clamp(3rem,5.3vw,6.5rem)] font-light leading-[0.96] tracking-[-0.045em]">{services("headline")}</h2></div>
          <div className="mt-24 space-y-28 lg:space-y-44">
            {stages.map((stage, index) => (
              <article key={stage.title} className={`mx-auto grid max-w-[104rem] items-center gap-10 px-6 lg:grid-cols-12 lg:px-10 ${index % 2 ? "" : ""}`}>
                <div className={`relative aspect-[4/3] overflow-hidden lg:col-span-7 ${index % 2 ? "lg:col-start-6" : ""}`}><Image src={stage.image} alt="" fill sizes="(min-width:1024px) 58vw,100vw" className="object-cover" /></div>
                <div className={`lg:col-span-4 ${index % 2 ? "lg:col-start-1 lg:row-start-1" : "lg:col-start-9"}`}><span className="mb-8 block h-px w-20 bg-[#8e583f]" /><h3 className="text-3xl font-light tracking-[-0.035em] sm:text-5xl">{stage.title}</h3><p className="mt-6 text-sm leading-8 text-black/58 sm:text-base">{stage.desc}</p></div>
              </article>
            ))}
          </div>
        </section>

        <section id="cinematic-range" className="relative min-h-[52rem] bg-[#0d1215] py-24 text-white lg:py-36">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#0d1215] via-[#0d1215]/90 to-[#0d1215]/20" />
          <div className="relative z-10 mx-auto max-w-[104rem] px-6 lg:px-10"><p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#d49a76]">{product("eyebrow")}</p><h2 className="mt-6 max-w-[13ch] text-[clamp(3.2rem,5.5vw,6.5rem)] font-light leading-[0.94] tracking-[-0.045em]">{product("headline")}</h2><p className="mt-7 max-w-xl text-base leading-8 text-white/55">{product("description")}</p><div className="mt-12 grid max-w-3xl gap-px bg-white/20 sm:grid-cols-2">{[[product("boxerBriefs"),product("boxerBriefsTag")],[product("trunks"),product("trunksTag")],[product("briefs"),product("briefsTag")],[product("performance"),product("performanceTag")]].map(([name,note]) => <div key={name} className="bg-[#0d1215]/90 p-6"><h3 className="text-xl font-medium">{name}</h3><p className="mt-2 text-sm leading-6 text-white/45">{note}</p></div>)}</div><Link href="/products" className="mt-10 inline-flex items-center gap-3 border-b border-white/50 pb-2 text-xs font-semibold uppercase tracking-[0.14em]">{product("viewAll")}{arrowIcon}</Link></div>
        </section>

        <section id="cinematic-process" className="bg-[#d8cec0] py-28 lg:py-44">
          <div className="mx-auto grid max-w-[104rem] gap-12 px-6 lg:grid-cols-[0.75fr_1.25fr] lg:px-10">
            <div className="lg:sticky lg:top-20 lg:self-start"><p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#8e583f]">{process("eyebrow")}</p><h2 className="mt-6 text-[clamp(3rem,5vw,5.8rem)] font-light leading-[0.95] tracking-[-0.045em]">{process("headline")}</h2><p className="mt-7 max-w-lg text-base leading-8 text-black/55">{process("description")}</p></div>
            <ol className="border-t border-black/25">{["s1","s2","s3","s4","s5","s6","s7"].map((key) => <li key={key} className="grid gap-4 border-b border-black/20 py-8 sm:grid-cols-[0.8fr_1.2fr]"><h3 className="text-2xl font-medium tracking-[-0.03em]">{process(`${key}Title`)}</h3><div><p className="text-sm leading-7 text-black/58">{process(`${key}Desc`)}</p><p className="mt-4 text-[10px] uppercase tracking-[0.17em] text-[#8e583f]">{process("outputLabel")} · {process(`${key}Del`)}</p></div></li>)}</ol>
          </div>
        </section>

        <section className="grid min-h-[48rem] bg-[#15222a] text-white lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="flex flex-col justify-center p-8 sm:p-12 lg:p-20"><p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#d49a76]">{cases("casesEyebrow")}</p><h2 className="mt-6 text-[clamp(3rem,5vw,5.8rem)] font-light leading-[0.95] tracking-[-0.045em]">{cases("casesHeadline")}</h2><p className="mt-7 text-base leading-8 text-white/55">{cases("case1Client")} · {cases("case1P1Title")}</p><div className="mt-12 border-t border-white/20 pt-7"><p className="text-sm leading-7 text-white/50">{capability("description")}</p></div></div>
        </section>

        <section className="bg-[#ede8df] px-6 py-28 lg:px-10 lg:py-40"><div className="mx-auto max-w-[104rem] text-center"><p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#8e583f]">{cta("eyebrow")}</p><h2 className="mx-auto mt-7 max-w-[15ch] text-[clamp(3.2rem,6vw,7rem)] font-light leading-[0.92] tracking-[-0.05em]">{cta("headline")}</h2><p className="mx-auto mt-7 max-w-2xl text-base leading-8 text-black/55">{cta("description")}</p><Link href="/contact" className="mt-10 inline-flex items-center gap-3 bg-[#15222a] px-8 py-5 text-xs font-semibold uppercase tracking-[0.14em] text-white">{cta("cardButton")}{arrowIcon}</Link></div></section>
      </main>

      <footer className="bg-[#0d1215] px-6 pb-28 pt-16 text-white lg:px-10"><div className="mx-auto grid max-w-[104rem] gap-10 border-t border-white/15 pt-10 sm:grid-cols-2"><div><p className="text-xl font-medium tracking-[0.02em]">ABSTRACT MAN</p><p className="mt-4 max-w-xl text-sm leading-7 text-white/40">{footer("description")}</p></div><div className="sm:text-right"><p className="text-sm text-white/45">{footer("location")}</p><a href="mailto:lei@abstractmanwear.com" className="mt-2 inline-block text-sm text-white">lei@abstractmanwear.com</a></div></div></footer>
    </div>
  );
}
